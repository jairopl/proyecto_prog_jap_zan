...
if (!function_exists("pluralize")) {
	function pluralize($word) {		
		...
		$plural = array("/(quiz)$/i" => "$1zes", "/^(ox)$/i" => "$1en",	"/([m|l])ouse$/i" => "$1ice", "/(matr|vert|ind)ix|ex$/i" => "$1ices", "/(x|ch|ss|sh)$/i" => "$1es", "/([^aeiouy]|qu)y$/i" => "$1ies", "/(hive)$/i" => "$1s", "/(?:([^f])fe|([lr])f)$/i" => "$1$2ves", "/(shea|lea|loa|thie)f$/i" => "$1ves", "/sis$/i" => "ses", "/([ti])um$/i"=> "$1a", "/(tomat|potat|ech|her|vet)o$/i" => "$1oes", "/(bu)s$/i" => "$1ses", "/(alias)$/i" => "$1es", "/(octop)us$/i" => "$1i", "/(ax|test)is$/i" => "$1es", "/(us)$/i" => "$1es",	"/s$/i" => "s", "/$/" => "s");	
